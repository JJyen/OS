...
public class Custom {
	...
	public Custom(ArrayList<Process> customjobQ) {
		this.jobQ = customjobQ;
		time = 0;
	}
	...
	public void run() {
		...
		int jobQIndex = 0;
		jobQ.get(jobQIndex).setTimeSlice(3);
		...
		while(!readyQ.isEmpty()) {
			...
			if(readyQ.element().getBurstTime() <= readyQ.element().getTimeSlice()) {
				//
				...
				//
				for(int i = 0; i < readyQ.element().getTimeSlice() - 2; i++) pidBuilder.append(" ");
				for(int i = 0; i < readyQ.element().getTimeSlice(); i++) timeBuilder.append(" ");
				
				time += readyQ.element().getTimeSlice();
				readyQ.element().setBurstTime(readyQ.element().getBurstTime() - readyQ.element().getTimeSlice());
				readyQ.element().setTimeSlice(readyQ.element().getTimeSlice()*2);
				for(int i = jobQIndex; i < jobQ.size(); i++) { //jobQ 에서 readyQ로
					if(time >= jobQ.get(jobQIndex).getArrivalTime()) {
						jobQ.get(jobQIndex).setTimeSlice(3);
						readyQ.add(jobQ.get(jobQIndex));
